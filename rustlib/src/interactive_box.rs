...
impl VelloApp for InteractiveBox {
    ...
    fn on_init(&mut self) {
        ...
        info!("[InteractiveBox::on_init] Application initialization started. Version: 1.0.0-example");

        // 设置初始坐标为 (500, 500)
        self.x = 500.0;
        self.y = 500.0;
        debug!("[InteractiveBox::on_init] Initial box coordinates set to x: {}, y: {}", self.x, self.y);
    ...
    fn on_resize(&mut self, width: u32, height: u32) {
        ...
        info!("[InteractiveBox::on_resize] New surface dimensions: {}x{}", width, height);

        // 更新屏幕尺寸
        self.screen_width = width as f32;
        self.screen_height = height as f32;
        debug!("[InteractiveBox::on_resize] Updated screen dimensions - width: {}, height: {}", self.screen_width, self.screen_height);

        // 将方块重新定位到屏幕中心
        self.x = self.screen_width / 2.0;
        self.y = self.screen_height / 2.0;
        debug!("[InteractiveBox::on_resize] Box repositioned to screen center: ({}, {})", self.x, self.y);

        trace!("[InteractiveBox::on_resize] Box position recalibrated to screen center: ({}, {})", self.x, self.y);
    ...
    fn on_touch(&mut self, action: i32, x: f32, y: f32) {
        ...
        trace!("[InteractiveBox::on_touch] Action: {}, Coordinates: ({:.2}, {:.2})", action, x, y);
        
        // 更新方块位置到触摸位置
        self.x = x;
        self.y = y;
        debug!("[InteractiveBox::on_touch] Box position updated to: ({:.2}, {:.2})", self.x, self.y);
    ...
    fn on_draw(&mut self, context: &mut RenderContext, dt: f64) {
        ...
        trace!("[InteractiveBox::on_draw] Current box position: ({:.2}, {:.2})", self.x, self.y);
        trace!("[InteractiveBox::on_draw] Screen dimensions: {:.2}x{:.2}", self.screen_width, self.screen_height);

        // 1. 绘制深灰色背景 (使用高级 Color API)
        debug!("[InteractiveBox::on_draw] Drawing deep gray background");
        let bg_color = Color::from_rgb8(30, 30, 30);
        debug!("[InteractiveBox::on_draw] Background color: RGB({}, {}, {})", 30, 30, 30);
        ...
        debug!("[InteractiveBox::on_draw] Background rectangle: {:?}", bg_rect);
        ...
        debug!("[InteractiveBox::on_draw] Box size: {}, half size: {}", box_size, half_size);
